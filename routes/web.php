...
Route::get('/sys-setup-secreto-' . env('APP_SETUP_TOKEN', '12345'), function () {
...
function demoFallbackData(): array
{
    return [
...
Route::get('/', function () {
    $siteUrl = url('/sitios/logistica-pro');

    try {
        $site = Site::query()->first();

        if ($site?->slug) {
            $siteUrl = url("/sitios/{$site->slug}");
        }
    } catch (\Throwable $exception) {
        Log::warning('No fue posible cargar el sitio principal para la portada.', [
            'message' => $exception->getMessage(),
        ]);
    }

    return view('home', compact('siteUrl'));
});

Route::get('/sitios/{slug}', function (string $slug) {
    $dbReady = true;
    $fallbackData = demoFallbackData();

    try {
        $site = Site::query()->where('slug', $slug)->first();

        if (! $site && $slug === $fallbackData['slug']) {
            $site = new Site($fallbackData);
            $dbReady = false;
        }
